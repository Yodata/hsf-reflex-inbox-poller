...
describe("getRecipients", () => {
  ...
  test("supports recipient.id or @id", () => {
    expect.assertions(3);
    expect(
      getRecipients({ recipient: [{ "@id": "test/profile/card#me" }] })
    ).toEqual(["test/inbox/"]);
    expect(
      getRecipients({ recipient: [{ id: "test/profile/card#me" }] })
    ).toEqual(["test/inbox/"]);
    expect(
      getRecipients({
        recipient: [{ id: "a/profile/card#me" }, { "@id": "b/profile/card#me" }]
      })
    ).toEqual(["a/inbox/", "b/inbox/"]);
  });

  test("recipient value can be string", () => {
    expect(getRecipients({ recipient: "test/profile/card#me" })).toEqual([
      "test/inbox/"
    ]);
    expect(getRecipients({ recipient: ["test/profile/card#me"] })).toEqual([
      "test/inbox/"
  ...
  test("recipient can reference inbox url or profile/card#me", done => {
    expect.assertions(2);
    expect(getRecipients({ recipient: [{ id: "test/inbox/" }] })).toEqual([
      "test/inbox/"
    ]);
    expect(
      getRecipients({ recipient: [{ "@id": "test/profile/card#me" }] })
  ...
  test("recipient can be an object", done => {
    expect(
      getRecipients({
        recipient: {
          "@id": "https://1047522.ds.bhhsresource.com/profile/card#me"
        }
      })
    ).toEqual(["https://1047522.ds.bhhsresource.com/inbox/"]);
    done();
  });
});
